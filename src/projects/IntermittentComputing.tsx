  ...
  return (
    <div className="py-5">
      ...
      <P>
        ...
        Embedded Wireless Systems and Networks (EWSN) Call For Competitors. The
        goal of this competition was to implement and evaluate different
        approaches to designing efficient intermittently-powered sensing
        systems. Ideally, this would advance the field of battery-free computing
      ...
      <P>
        ...
        organizers will emulate varying degrees of available energy. To solve a
        challenge, contestants have to utilize hashcash, a SHA1-based
        proof-of-work algorithm for which a reference implementation is provided
        by the organizers.
      </P>
      <br />
      <P>
        <b>Example:</b> Using a challenge 'EWSN2024' and a difficulty of 16-bit,
        we first create a starting string with a well-known format, e.g.,{" "}
        <i>1:16:240403:EWSN2024::WXhnFeDleN:1</i>. The segments of the string
        separated by a colon are: version (always 1), bits (16 as per
        ...
        string), and a counter. We now need to check if this string's SHA1 hash
        has 16 (as per the chosen difficulty) leading zeros (most significant
        bits). Our string (with the counter of 1) has a SHA1 hash of{" "}
        <i>20db26cb6b1e17a2079fc3daf05fd01a7ed08cb5</i>, which only has two
        leading zeros. Thus, we increment the counter to 2 and hash the updated
        string and keep incrementing the counter until we find a string that has
        a hash with the required leading zeros. In this example, after 96620
        attempts, the string with counter 96620{" "}
      ...
      <br />
      <P>
        At the start of an experiment, an external I2C FRAM memory will contain
        a list of challenges similar to the above example, to which contestants
        need to supply a solution despite the intermittent energy supply. At the
        end of an experiment, the computed solutions are automatically collected
        by E-Cube, the number of correct solutions is tallied against any
        incorrect solutions, and points awarded accordingly. Additional
        technical details, a reference solution, as well as the evaluation
        procedure will be posted on the competition's website.
      </P>
      <ST>Introduction</ST>
      <P>
        ...
        remain steady in the next clock cycle. We want to practice implementing
        novel approaches in scenarios that may corrupt the system or compromise
        the validity of output data.
      </P>
      <ST>Constraints</ST>
      <P>
      ...
      <P>
        ...
        hashing algorithm. The goal is to find a string that, when hashed, has a
        certain number of leading zeros. When this string is found, the
      ...
      <P>
        <b>Long</b>: Just go to{" "}
        <a href={hashcashLink} className="underline text-blue-500">
          Wikipedia
        </a>
        .
      </P>
      ...
      <NL>
        <li>
          ...
          primary inhibitor to intermittent computing, i.e. how can we design a
          system that will remain consistent through power failures? We were
          really inspired by{" "}
          <a href={WARioPaperLink} className="underline text-blue-500">
            WARio
          </a>
          , the custom compiler instruction approach by <i>Kortbeek, et.al.</i>
        </li>
        <li>
        ...
        <li>
          <b>State Saving:</b> We know ahead of time that the system is going to
          have a power failure by design. Therefore, the code needs to have a
          reliable method to continue computations without completely restarting
          from the top.
        </li>
      </NL>
      <ST>Handling WAR Hazards</ST>
      <P>
        Due to the importance of consistency, Write After Read (WAR) hazards are
        at the heart of the intermittent computing challenge. WAR hazards occur
        when a subsequent write operation modifies a value that has not been
        fully read by a prior operation, leading to inconsistent states
      ...
      <P>
        ...
        program state effectively and saving at crucial moments. Managing the
        state of the system across power interruptions required a robust
      ...
      <P>
        ...
        iterations. This structure was saved to NVM in batches after each atomic
        function to try and keep the variables as updated as possible.
      </P>
      <br />
      <P>
      ...
      <P>
        ...
        the SHA-1 computation to complete before continuing to the next step in
        the algorithm.
      </P>
      <br />
      <P>
        ...
        resume from any function call without reprocessing previously completed
        work. We also aimed to minimize work within each function to promote
        frequent variable updates.
      </P>
      <br />
      <P>
        <b>Instruction Pointer Flag:</b> An instruction pointer flag is used to
        check which function was currently running when power was shut off. On
        startup, the program will continue the algorithm from the last function
        to ensure minimal data loss and avoid unnecessary computing.
      </P>
      <ST>Results</ST>
      <P>
        ...
        hashcash algorithm, so we didn't get a submission in to the competition.
      </P>
      <br />
      <P>
        Overall, I'm a little sad we didn't get to actually participate but I
        had a decent idea that we weren't going to have a shot, especially since
        we're going to be competing against full time PhD students and
...
export const IntermittentComputing = {
  ...
  tags: ["C++", "Embedded Systems", "MSP430"],
  body: <Intermittent />,
};
