...
export function HomeProjectCard({
  ...
  const cardContent = (
    <div
      ...
      className={`relative border-2 border-slate-800/70 bg-white font-extralight h-24 lg:h-32 rounded-lg flex items-center text-2xl sm:text-3xl hover:cursor-pointer ${color.text} transition-all duration-300 p-10 hover:-translate-y-0.5 hover:shadow-md flex justify-start sm:justify-center select-none`}
    >
      {name}
      {opensNewTab ? (
        <span
