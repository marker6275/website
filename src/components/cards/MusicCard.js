...
export function MusicCard({ id, title, link, month, year, composer, channel }) {
  ...
  return (
    <a href={channel} target="_blank" rel="noreferrer">
      <div
        className={`bg-[#FCF5E5] w-[360px] md:w[360px] lg:w-[472px] xl:w-[600px] shadow-md flex flex-col items-center justify-center rounded-xl border-4 ${colors[id][0]} ${colors[id][1]} ${colors[id][2]} transition duration-400`}
      >
