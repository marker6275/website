...
export default function Calculator() {
  ...
  const [newValue, setNewValue] = useState<boolean>(true);
  const [view, setView] = useState<"Number" | "Text">("Number");
  ...
  function onOperationClick(display: string, operation: string) {
    if (operation === "+/-") {
    ...
    if (operation === "sqr") {
      setDisplay(String(parseFloat(display) * parseFloat(display)));
      return;
    }
    ...
    if (currentOperation) {
      ...
      switch (currentOperation) {
        case "+":
          result = String(Math.round((a + b) * 10000) / 10000);
          break;
        case "-":
          result = String(Math.round((b - a) * 10000) / 10000);
          break;
        case "*":
          result = String(Math.round(a * b * 10000) / 10000);
          break;
  ...
  function calculateText(A: number, B: number | undefined, operation: string): number {
    ...
    const second = parseFloat(String(B));
